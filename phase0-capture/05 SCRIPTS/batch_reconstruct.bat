...
:: (LOCAL EDIT) COLMAP 3.12.3 ships its Qt platform plugin (qwindows.dll)
:: under \plugins. Without QT_PLUGIN_PATH the feature_extractor and
:: sequential_matcher fail with "Could not find the Qt platform plugin
:: windows" and write nothing to the database. This mirrors COLMAP.bat.
...
:: (LOCAL EDIT) --SiftMatching.use_gpu 0 forces CPU matching. On this AMD
:: RX 7600 box (with a Parsec Virtual Display Adapter present) COLMAP's
:: OpenGL SIFT matcher grabs the virtual adapter and dies with "Not enough
:: GPU memory to match N features". CPU matching is slower but reliable.
:: On a healthy NVIDIA box you can drop this flag for GPU-speed matching.
